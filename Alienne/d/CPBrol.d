...
END
+ ~Global("CPBeregostSearch","GLOBAL",2)Class(LastTalkedToBy,MAGE_ALL)~ + ~Ich bin auf der Suche nach einem vermissten Mädchen, vielleicht könntet ihr mir helfen.~ + Mage
+ ~Global("CPBeregostSearch","GLOBAL",2)!Class(LastTalkedToBy,MAGE_ALL)~ + ~Ich bin auf der Suche nach einem vermissten Mädchen, vielleicht könntet Ihr mir helfen.~ + Girl
+ ~Global("CPBeregostSearch","GLOBAL",2)~ + ~Hütet Eure Zunge, oder ich schneide sie Euch heraus!~ + Fight
++ ~Verzeiht die Störung, ich werde Euch in Ruhe lassen.~ + EndTalk

CHAIN
IF ~NumTimesTalkedToGT(0)~ THEN CPBrol Talk2
~Ihr schon wieder? Verschwindet, <RACE>, bevor ihr meine Stiefel zu spüren bekommt!~
END
+ ~Global("CPBeregostSearch","GLOBAL",2)Class(LastTalkedToBy,MAGE_ALL)~ + ~Ich bin auf der Suche nach einem vermissten Mädchen, vielleicht könntet ihr mir helfen.~ + Mage
+ ~Global("CPBeregostSearch","GLOBAL",2)!Class(LastTalkedToBy,MAGE_ALL)~ + ~Ich bin auf der Suche nach einem vermissten Mädchen, vielleicht könntet Ihr mir helfen.~ + Girl
+ ~Global("CPBeregostSearch","GLOBAL",2)~ + ~Hütet Eure Zunge, oder ich schneide sie Euch heraus!~ + Fight
...
CHAIN
IF ~~ THEN CPBrol Mage
~(Brols gesamte Aufmerksamkeit richtet sich plötzlich auf Euch. Sein Blick wandert prüfend über Eure Kleidung.)~
...
+ ~CheckStatGT(LastTalkedToBy,16,39)~ + ~ Ohne Euch zu nahe treten zu wollen, aber durch Euer ablehnendes Verhalten macht Ihr Euch äußerst verdächtig.~ + Convince
+ ~CheckStatLT(LastTalkedToBy,17,39)~ + ~ Ohne Euch zu nahe treten zu wollen, aber durch Euer ablehnendes Verhalten macht Ihr Euch äußerst verdächtig.~ + ConvinceFail
+ ~Global("CPCorrupt100","GLOBAL",0)~ + ~Nun, vielleicht kann das Klimpern von 100 Goldmünzen euch ja umstimmen?~ DO ~SetGlobal("CPCorrupt100","GLOBAL",1)TakePartyGold(100)~ + CorruptFail100
+ ~Global("CPCorrupt300","GLOBAL",0)~ + ~Wie wär’s mit 300 Goldmünzen, würde das Eure Zunge lockern?~ DO ~SetGlobal("CPCorrupt300","GLOBAL",1)TakePartyGold(300)~ + CorruptFail300
++ ~Der Klang von 500 Goldmünzen sollte selbst in Euren Ohren wie Musik klingen, was wisst Ihr?~ DO ~TakePartyGold(500)~ + Corrupt
++ ~Verzeiht die Störung, ich werde Euch in Ruhe lassen..~ + EndTalk

CHAIN
IF ~~ THEN CPBrol Threat
~Schon gut <PRO_GIRLBOY>, kein Grund gleich ungemütlich zu werden! Wir wollen doch hier keinen Ärger anfangen, oder?
Jetzt steckt die Waffen wieder ein, ich erzähle Euch was ich weiß.~
END CPBrol Explain

CHAIN
IF ~~ THEN CPBrol ThreatFail
~(Brol lacht laut) Macht Euch nicht lächerlich, sogar der fette Schankwirt würde Euch im Handumdrehen vor die Tür jagen! Nun steckt die Waffen ein bevor Ihr Euch noch verletzt.~
END
+ ~CheckStatGT(LastTalkedToBy,16,39)~ + ~ Ohne Euch zu nahe treten zu wollen, aber durch Euer ablehnendes Verhalten macht Ihr Euch äußerst verdächtig.~ + Convince
+ ~CheckStatLT(LastTalkedToBy,17,39)~ + ~ Ohne Euch zu nahe treten zu wollen, aber durch Euer ablehnendes Verhalten macht Ihr Euch äußerst verdächtig.~ + ConvinceFail
+ ~Global("CPCorrupt100","GLOBAL",0)~ + ~Nun, vielleicht kann das Klimpern von 100 Goldmünzen euch ja umstimmen?~ DO ~SetGlobal("CPCorrupt100","GLOBAL",1)TakePartyGold(100)~ + CorruptFail100
...
~*grml* Ich nehme an, dass Ihr selber an Euer dummes Geschwätz glaubt. Aber ich kann es mir nicht leisten mit der Wache in Konflikt zu geraten.~
END CPBrol Explain

CHAIN             
IF ~~ THEN CPBrol ConvinceFail
~(Brol lacht laut) Verdächtig, ja? Also ist jeder hier der von Eurem Geschwätz genervt ist "verdächtig"?. Was wollt Ihr tun, die Wache rufen? Zieht Leine!~
END
+ ~CheckStatGT(LastTalkedToBy,16,36)~ + ~ Nun, wir haben nicht gerade viel Zeit und haben ebenso wenig Lust zu plaudern wie Ihr. Aber im Gegensatz zu euch sind wir schwer bewaffnet und kampfbereit.~ + Threat
+ ~CheckStatLT(LastTalkedToBy,17,36)~ + ~ Nun, wir haben nicht gerade viel Zeit und haben ebenso wenig Lust zu plaudern wie Ihr. Aber im Gegensatz zu euch sind wir schwer bewaffnet und kampfbereit.~ + ThreatFail
+ ~Global("CPCorrupt100","GLOBAL",0)~ + ~Nun, vielleicht kann das Klimpern von 100 Goldmünzen euch ja umstimmen?~ DO ~SetGlobal("CPCorrupt100","GLOBAL",1)TakePartyGold(100)~ + CorruptFail100
...
~Es fällt mir weiterhin sehr schwer mich zu erinnern, wenn Ihr versteht was ich meine.
...
~Ja, ja, ja! Ich erinnere mich dunkel etwas über ein Elfenmädchen gehört zu haben! Wenn ich mich doch nur entsinnen könnte wo...
...
~Ahhh, natürlich! Ich erinnere mich wieder!~
END CPBrol Explain

CHAIN
IF ~~ THEN CPBrol Explain
~Meine Auftraggeber haben mich nach Beregost geschickt um hier ihre Interessen zu vertreten. In den nächsten Tagen sollte ich mich mit meinem Kontaktmann treffen um alles weitere zu vereinbaren.~
...
~(Brol schnaubt verächtlich) Was glaubt Ihr eigentlich wer Ihr seid? Anscheinend muss ich das Gesindel von der Straße wieder einmal in die Schranken weisen.~
END
